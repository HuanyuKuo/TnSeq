...
class Dilution:
    ...
    def get_experimental_s(self):
        ...
        env = '"' + self.replica + '_' + self.media + '"'
        strain = '"' + self.strain + '"'
        col = 7
        fopen = open(filename,'r')  # open file
        dat = fopen.read().split('\n')  # read file matrix and save in 1-dimension dat
        fopen.close() # close file
        row = int(len(dat)/col)   # get length of a coloum (number of row)
        #self.s_dat = np.resize(self.s_dat,(row,col))  # resize the 1-dimension dat to matrix 
        sdat = []
        for i in range(0,row):
            ...
            tmp2 = tmp.split(',')
            sdat.extend(tmp2)
        sdat = np.resize(sdat,(row,col))
        #self.s_dat = []
        #print env, strain
        for i in range(0,row):
            if sdat[i][2]==env and sdat[i][3]==strain:
                #print i
                s1 = sdat[i][1] #bcid
                s2 = sdat[i][4] #experimental s
                s3 = sdat[i][6] # r squared
                f1 = np.int32(s1[1:-1])
                f2 = np.float64(s2[1:-1])
                f3 = np.float64(s3[1:-1])
                #print s1, f1
                self.s_bcid.append(f1)
                self.s_dat.append(f2)
                self.s_r2.append(f3)
         '''
    '''
    def input_dat(self, file, col):
        fopen = open(file,'r')  # open file
        dat = fopen.read().split()  # read file matrix and save in 1-dimension dat
        row = int(len(dat)/col)   # get length of a coloum (number of row)
        dat = np.resize(dat,(row,col))  # resize the 1-dimension dat to matrix 
        fopen.close() # close file
        return dat, row
    '''
    ...
    def read_files(self):
        ...
        '''
            # Record initial barcode freqnecy as "real" probability for samping, make it in decending order
            if gen ==0:
                pvals = bc_count/float(self.all_count[gen]) # Record initial barcode freqnecy as "real" probability pvals
                self.sort_index = np.argsort(bc_count) 
                self.sort_index = self.sort_index[::-1] # reverse
                self.pvals = pvals[self.sort_index] # make it in decending order
                self.N_BCID = len(self.pvals)
            ...
            for ref in range(0,2):
                idx1 = self.get_index(bc_id, self.ref_bcid[ref])  # index of reference barcode in original file
                self.ref_counts[ref][gen] = bc_count[idx1]
                if gen ==0:
                    idx2 = self.get_index(self.sort_index, idx1)      # index of sort_of_original_index 
                    self.ref_idx.append(idx2)
