...
class Dilution:
    ...
    def runs(self):
        ...
        self.s_record = self.selection_coef(simu_scaled_reads)
    ...
    def selection_coef(self, scaled_reads):
        ...
        s_record = [0 for i in range(0,self.N_BCID)]
        x = np.arange(0,45,10)
    ...
    def sort_s_for_plot(self):
        
        sort_indx = np.argsort(self.s_record)
        #sort_indx = sort_indx[::-1]  
        tmp = np.array(self.s_record)
        tmp = tmp[sort_indx]
    ...
    def plot_s_dist(self):  # Optional
        UPBOUND, s_plot = self.sort_s_for_plot()
